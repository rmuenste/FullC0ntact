...
void pyramidtest()
{
  ...
  pos = VECTOR3(myGrid.m_vMax.x/2.0-2.0*d, myGrid.m_vMax.y/2.0, 7.25 * extends[2]);
...
void spherestack()
{
  ...
  std::cout<<"Number of particles in perrowx: "<<perrowx<<std::endl;      
  std::cout<<"Number of particles in layer: "<<numPerLayer<<std::endl;    
  std::cout<<"Number of particles total: "<<nTotal<<std::endl;
  ...
  //VECTOR3 pos(myGrid.m_vMin.x+drad+distbetween , myGrid.m_vMax.y/2.0, (myGrid.m_vMax.z/1.0)-d);
  VECTOR3 pos(myGrid.m_vMin.x+drad+distbetween , myGrid.m_vMin.y+drad+distbetween+0.0025, (myGrid.m_vMin.z+drad));
...
void initsimulation()
{
  ...
  //set the timestep
  myTimeControl.SetDeltaT(0.0025);
