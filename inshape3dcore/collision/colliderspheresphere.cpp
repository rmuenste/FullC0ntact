...
void CColliderSphereSphere::Collide(CRigidBody *pBody0, CRigidBody *pBody1, std::vector<CContact> &vContacts, Real dDeltaT)
{
  const Real contactTolerance = 0.00005;
	VECTOR3 &vel1 = pBody0->m_vVelocity;
	VECTOR3 &pos1 = pBody0->m_vCOM;

	CSphere<Real> *pSphere = dynamic_cast<CSphere<Real>* >(pBody0->m_pShape);
	Real rad1 = pSphere->Radius();

	VECTOR3 &vel2 = pBody1->m_vVelocity;
	VECTOR3 &pos2 = pBody1->m_vCOM;

	pSphere = dynamic_cast<CSphere<Real>* >(pBody1->m_pShape);
  ...
  Real distpertime = (dist1+dist2)*dDeltaT;  
...
void CColliderSphereSphere::Collide(std::vector<CContact> &vContacts, Real dDeltaT)
{
  const Real contactTolerance = 0.00005;
