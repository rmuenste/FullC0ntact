...
void CColliderBoxBoxBoundary::Collide(CRigidBody *pBody0, CRigidBody *pBody1, std::vector<CContact> &vContacts, Real dDeltaT)
{
  ...
  COBB3r newbox = Transform.PredictMotion(origBox0,pBody0->m_vVelocity,pBody0->GetTransformation(),pBody0->GetAngVel(),dDeltaT);
  ...
  Real radius = newbox.GetBoundingSphereRadius();
    ...
    for(i=0;i<8;i++)
    {
      ...
      VECTOR3 angPart = (VECTOR3::Cross(pBody0->GetAngVel(),vertices[i]-pBody0->m_vCOM));
      VECTOR3 relativeVelocity = (pBody0->m_vVelocity + angPart);
...
void CColliderBoxBoxBoundary::Collide(std::vector<CContact> &vContacts, Real dDeltaT)
{
