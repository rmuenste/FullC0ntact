...
template <typename T>
Segment3<T> OBB3<T>::getRegionEdge(unsigned int iRegion) const
{
...
template <typename T>
Rectangle3<T> OBB3<T>::getRegionFace(unsigned int iRegion) const
{
  ...
  switch(iRegion)
  {
    case 1:
      ...
      rec.uv_[0]=vAxes[1];
      rec.uv_[1]=vAxes[2];
      rec.extents_[0]=extents_[1];
      rec.extents_[1]=extents_[2];
      break;
    case 2:
      rec.center_= extAxis0;
      rec.uv_[0]=vAxes[1];
