    ...
    private:
        ...
        inline void checkDistanceCriterion(const Hexa &hex, const Vector3<T> &mid, int ive) {
            ...
            for(auto vidx = 0; vidx < 8; ++vidx) {
                ...
                if(0.5 * distMid > grid_->elementDist_[ive]) {
...
template<typename T>
class MeshDecimaterIntersection
{
public:
	...
	MeshDecimaterIntersection(void) {

    }

	MeshDecimaterIntersection(UnstructuredGrid<T,DTraits> *grid, MeshObject<T, cgalKernel> *meshObj) : grid_(grid), meshObj_(meshObj) {

    }

	virtual ~MeshDecimaterIntersection(void) {
