...
template <typename T>
CDistanceLineRec<T>::CDistanceLineRec(const CLine3<T>& line, const CRectangle3<T>& rec)
{
	this->m_Line = line;
	this->m_Rec  = rec;
}
...
T CDistanceLineRec<T>::ComputeDistanceSqr()
{
	...
	// zero.
	CVector3<T> N = CVector3<T>::Cross(m_Rec.m_vUV[0],m_Rec.m_vUV[1]);
	T NdD = N * m_Line.m_vDir;
	if (fabs(NdD) > E5)
	{
		// The line and rectangle are not parallel, so the line intersects
		// the plane of the rectangle.
		CVector3<T> diff = m_Line.m_vOrigin - m_Rec.m_vCenter;
		CVector3<T> U, V;
		CVector3<T>::GenerateComplementBasis(U, V, m_Line.m_vDir);
		T UdD0 = U * m_Rec.m_vUV[0];
		T UdD1 = U * m_Rec.m_vUV[1];
		T UdPmC = U * diff;
		T VdD0 = V * m_Rec.m_vUV[0];
		T VdD1 = V * m_Rec.m_vUV[1];
		T VdPmC = V * diff;
		...
		if (fabs(s0) <= m_Rec.m_Extents[0]
		&&  fabs(s1) <= m_Rec.m_Extents[1])
		{
			// Line parameter for the point of intersection.
			T DdD0 = m_Line.m_vDir * m_Rec.m_vUV[0];
			T DdD1 = m_Line.m_vDir * m_Rec.m_vUV[1];
			T DdDiff = m_Line.m_vDir * diff;
			...
			m_vClosestPoint0 = m_Line.m_vOrigin + m_ParamLine*m_Line.m_vDir;

			m_vClosestPoint1 = m_Rec.m_vCenter + s0*m_Rec.m_vUV[0] + s1*m_Rec.m_vUV[1];
	...
	CVector3<T> scaledDir[2] =
	{
			m_Rec.m_Extents[0]*m_Rec.m_vUV[0],
			m_Rec.m_Extents[1]*m_Rec.m_vUV[1]
	};
	for (int i1 = 0; i1 < 2; ++i1)
	{
		...
		{
			CSegment3<T> segment;
			segment.m_vCenter = m_Rec.m_vCenter +
					((T)(2*i0-1))*scaledDir[i1];
			segment.m_vDir = m_Rec.m_vUV[1-i1];
			segment.m_Ext = m_Rec.m_Extents[1-i1];
			...
			if (sqrDistTmp < sqrDist)
			{
					...
					T ratio = queryLS.m_ParamSegment/segment.m_Ext;
					m_ParamRectangle[0] = m_Rec.m_Extents[0]*((1-i1)*(2*i0-1) +
							i1*ratio);
					m_ParamRectangle[1] = m_Rec.m_Extents[1]*((1-i0)*(2*i1-1) +
							i0*ratio);
			}
		}
	}
